...
3. Correlation structure [7]: </br>
    a. Low pair-wise correlations between cells but significant high-order correlations.</br>
    b. High order assembly correlated with specific point in a predictable sequence. </br>
    c.  Unanticipated inputs lead to a burst of activity correlated vertically within mini-columns.</br>
    d.  Activity during predicted inputs will be a subset of activity during unpredicted inputs.</br>
    e.  Predictable inputs lead to negative correlations within mini-columns.
    f. Neighboring mini-columns will be uncorrelated.</br>

4. Branch specific plasticity [8]:

    a. Strong LTP in dendritic segments: NMDA spike followed by back action potential (bAP).</br>
    b.  Weak LTP (without NMDA spike) if synapse cluster becomes active followed by a bAP.</br>
    c.  Weak LTD when an NMDA spike is not followed by an action potential/bAP.
